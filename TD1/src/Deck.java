...
public class Deck {
	...
	public Integer pick(Deck d) {
		...
		Integer n = d.pack_of_cards.removeFirst();
		this.pack_of_cards.add(n);
		return n;
	}
	...
	public boolean isDeck(int nbVals) {
		...
		for (Integer i : this.pack_of_cards) {
			if (i>nbVals || i == 0) return false;
			tab[i-1] ++;
			if (tab[i-1]==5) return false;
		}
		return true;
	}
	...
	public void riffleWith(Deck d) {
		...
		if(temp.isEmpty()) this.pickAll(d);
		else this.pickAll(temp);
	}
